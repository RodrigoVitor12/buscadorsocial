        ...
        try {
            ...
            foreach ($redes as $rede) {
                ...
                $response = Http::withHeaders([
                    'X-API-KEY' => env('SERPER_API_KEY'), // ← Mude o nome da env se quiser
                    'Content-Type' => 'application/json',
                ])->post('https://google.serper.dev/search', [
                    'q' => $query,
                    'gl' => 'br',
                    'hl' => 'pt',
                    'location' => 'Brazil', // ajuda bastante no contexto BR
                    'start' => $start,
                    'num' => 10, // ou 20 se quiser mais por chamada
                    // Opcional: filtro por data (formato diferente do tbs do SerpApi)
                    // 'tbs'  => 'cdr:1,cd_min:1/1/2026,cd_max:12/31/2026',  // funciona, mas formato americano
                ]);

                if ($response->successful()) {
                    $data = $response->json();

                    $redeResults = collect($data['organic'] ?? [])
                        ->map(
                            fn($item) => [
                                'titulo' => $item['title'] ?? '',
                    ...
                    // \Log::error('Serper falhou', ['status' => $response->status(), 'body' => $response->body()]);
                    throw new \Exception('Falha na API Serper: ' . $response->status());
                }
            }

            // Ordenação alfabética pelo título (mantive igual)
            ...
            // \Log::error($e);  // recomendado em produção
        }
    }
...
<div class="text-white max-w-3xl mx-auto py-10">
    ...
    <div class="space-y-4">
        ...
        <div wire:loading class="mt-8">
            ...
            <div class="w-8 h-8 border-4 border-yellow-500 border-t-transparent rounded-full animate-spin mx-auto">
            </div>
